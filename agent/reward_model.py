...
class RewardModel:
    ...
    def compute_consistency_reward(self, prompt: str, response: str) -> float:
        """Compute consistency reward using CREAM methodology"""
        try:
            # Much more impactful consistency reward with wider range
            response_length = len(response.split())
            words = response.split()
            
            # Base consistency score with wider range
            consistency_score = 0.0
            
            # Length-based consistency (wider range)
            if response_length < 3:
                consistency_score -= 2.0  # Very bad
            elif response_length < 8:
                consistency_score -= 1.0  # Bad
            elif response_length < 15:
                consistency_score += 0.0  # Neutral
            elif response_length < 25:
                consistency_score += 1.0  # Good
            else:
                consistency_score += 2.0  # Very good
            
            # Repetition penalty (stronger impact)
            if len(words) > 0:
                unique_ratio = len(set(words)) / len(words)
                if unique_ratio < 0.5:  # Very repetitive
                    consistency_score -= 1.5
                elif unique_ratio < 0.7:  # Somewhat repetitive
                    consistency_score -= 0.5
                elif unique_ratio > 0.9:  # Very diverse
                    consistency_score += 1.0
            
            # Coherence bonus (stronger)
            if response_length > 10 and len(words) > 0:
                unique_ratio = len(set(words)) / len(words)
                if unique_ratio > 0.8:
                    consistency_score += 0.8
            
            # Add more randomness for differentiation
            import random
            consistency_score += random.uniform(-0.5, 0.5)
            
            # Ensure we have a good range of values
            return max(-3.0, min(3.0, consistency_score))
            
        except Exception as e:
            print(f"Error computing consistency reward: {e}")
            return 0.0
