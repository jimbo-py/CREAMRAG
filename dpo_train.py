...
class DPOCustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Compute DPO rewards (beta is applied in the loss, not here)
        chosen_rewards = chosen_log_probs - ref_chosen_log_probs
        rejected_rewards = rejected_log_probs - ref_rejected_log_probs
        ...
        # Compute DPO loss with proper beta scaling
        dpo_logits = self.config.beta * (chosen_total_rewards - rejected_total_rewards)
        dpo_loss = -F.logsigmoid(dpo_logits).mean()
        
        # Debug: Check if loss is valid
        if torch.isnan(dpo_loss) or torch.isinf(dpo_loss):
            logger.warning(f"Invalid DPO loss detected: {dpo_loss}")
            dpo_loss = torch.tensor(0.0, requires_grad=True, device=dpo_loss.device)
        
        # Debug: Log loss values
        if dpo_loss.requires_grad:
            logger.info(f"DPO Loss: {dpo_loss.item():.6f}, Chosen rewards: {chosen_total_rewards.mean().item():.6f}, Rejected rewards: {rejected_total_rewards.mean().item():.6f}")
    ...
    def _compute_log_probs(self, logits, input_ids):
        ...
        # Return sum log probabilities (correct for DPO)
